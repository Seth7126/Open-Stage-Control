...
- bug fixes
    - remote control: add support for icon transform suffixes

- widgets
    - encoder: add `horizontal` mode
    - knob: add `encoder` design

## 1.30.3

- bug fixes
    - scrollbar: missing update upon target container resize
    - server: regression breaking reception of osc message over tcp

- widgets
    - file: remember last visited directy per widget if `directory` property is not set
    - knob: add `horizontal` mode
    - eq: filter responses are now only computed when needed; the widget's value is no longer exposed / sync'ed.  
    - plot, eq: unclip line drawing on y-axis

- scripting
    - add `uploadFile` function for reading client-side files
    - add `scrollIntoView` function

## 1.30.2

- bug fixes
    - when using objects in properties, numbers-as-strings and booleans-as-strings values are now preserved instead of being converted to their litteral counterparts
    - rgb: fix rendering issue
    - patchbay: interaction and rendering issues
    - `event.preventDefault` in `onTouch` now affect modal buttons
    - menu: use click event instead of tap in `toggle` and `doubleTap-toggle` modes
    - button: missing `move` events in `onTouch`
    - error when logging `event` object in `onTouch`

- widgets
    - modal/button/file: add `click` property

- editor
    - preserve code editor undo history between consecutive edits of a same widget's property
    - preserve fullscreen code editor focus when saving / submitting

- client
    - add `noWarning` client option

## 1.30.1

- bug fixes
    - regression breaking user asset loading from a root directory named "assets"
    - regression breaking custom module's `nativeRequire()` for locally installed modules
...
- bug fixes
    ...
    - make the client app more robust to restrictive browser settings (cookie must still be enabled for the app to fully work)
...
- widgets
    ...
    - button: add `onTouch` script
    - switch: make `values` property dynamic

- editor
    - selecting a widget automatically shows it in the widget tree if it's not minimized
    - submitting an unchanged value for the `value` property will now affect the value state of the widget even if the property content itself did not change
...
- scripting
    ...
    - calling `setFocus()` without argument now unfocuses current active element

- midi
    - add `no_auto_note_off` option
    - add `Restart backend` action to the launcher's menu
    - add `restartMidiBackend()` function to custom module context
...
- misc
    - now using electron v36.9.5

## 1.29.9

- build
    - fix building from sources now that open stage control is not hosted on github

- scripting
    - user scripts are longer protected against infinite loop
