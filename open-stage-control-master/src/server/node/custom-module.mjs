...
class CustomModule {
    ...
    constructor(file, context, parent=null, mainModule=null) {
        ...
        if (!this.submodule && !modulePathExtended) {
            process.env.NODE_PATH += path.delimiter + path.resolve(path.dirname(this.filename))
            process.env.NODE_PATH += path.delimiter + path.resolve(path.dirname(this.filename), 'node_modules')
            Module._initPaths()
