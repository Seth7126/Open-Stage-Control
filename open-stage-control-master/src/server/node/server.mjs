...
class WebServer extends EventEmitter {
    ...
    httpRoute(req, res) {
        ...
        if (url === '/qrcode'){
        ...
        } else if (url.indexOf('__APP_THEME__.css') != -1) {
        ...
        } else  if (url.match(/^\/+__APP_DIR__\//)) {

            // osc asset files

            try {
                this.sendFile(req, res, path.resolve(__dirname + '/../../' + url.replace('__APP_DIR__/', '')))
                return true
            } catch(e) {}
            ...
            if (url.match(/.(svg|jpg|jpeg|png|apng|gif|webp|tiff|xbm|bmp|ico|ttf|otf|woff|woff2|html|css|js)(\?[0-9]*)?$/i)) {
                ...
                if (resolvedPath) {
                    try {
                        this.sendFile(req, res, resolvedPath)
                        return true
                    } catch(e) {}
                }
