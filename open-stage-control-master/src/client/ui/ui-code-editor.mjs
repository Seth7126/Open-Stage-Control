...
import scriptVm from '../widgets/scripts/script-vm.mjs'
...
ace.config.set('basePath', '/__APP_DIR__/client/ace/')
...
class CodeEditor {
    ...
    bind(widget, field) {
        ...
        this.editor.on('focus', (e)=>{
            ...
            setTimeout(()=>{
                if (this.pos) {
                    this.editor.moveCursorTo(this.pos.row, this.pos.column)
                } else {
            ...
            this.editor.on('blur', (e)=>{
                this.pos = this.editor.getCursorPosition()
                this.editor.setHighlightActiveLine(false)
                this.editor.setHighlightGutterLine(false)
        ...
        if (this.hash !== widget.hash) {
            // preserve undo history if widget is the same
            editor.getSession().getUndoManager().reset()
            this.hash = widget.hash
            this.pos = null
        } else if (editor.fullscreen) {
            this.fullscreen()
        }
