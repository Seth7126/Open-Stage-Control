...
class Knob extends Slider {
    ...
    static defaults() {
        ...
        var defaults = super.defaults(Knob).extend({
            style: {
                ...
                design: {type: 'string', value: 'default', choices: ['default', 'solid', 'line', 'encoder'], help: [
                    'Design style',
            ...
            class_specific: {
                mode: {type: 'string', value: 'vertical', choices: ['vertical', 'horizontal', 'circular', 'snap', 'snap-alt'], help: [
                    '- `vertical`: relative move in vertical motion',
                    '- `horizontal`: relative move in horizontal motion',
    ...
    dragHandle(e) {
        ...
        } else if ((this.getProp('mode') === 'horizontal' && !e.traversing)) {
            // horizontal
            this.percent = 100 * (e.movementX / inertia * this.getProp('sensitivity')) / this.minDimension + this.percent
    ...
    draw() {
        ...
        switch (this.getProp('design')) {
            ...
            case 'encoder':
            {
                ...
                if (this.cssVars.alphaFillOff) {
                    this.ctx.globalAlpha = this.cssVars.alphaFillOff
                    this.ctx.beginPath()
                    this.ctx.arc(this.width / 2, this.height / 2, gaugeRadius, min - angle, max + angle)
                    this.ctx.stroke()
                }
        ...
        if (pips) {
            ...
            for (var p in this.pipTexts) {
                ...
                let angle = this.percentToAngle(p),
