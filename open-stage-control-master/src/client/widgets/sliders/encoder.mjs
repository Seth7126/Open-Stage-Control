...
export default class Encoder extends StaticProperties(Knob, {angle: 360, range: {min: 0, max: 1}, design: 'encoder'}) {
    ...
    static defaults() {
        ...
        var defaults = super.defaults().extend({
            ...
            class_specific: {
                ...
                mode: {type: 'string', value: 'circular', choices: ['circular', 'snap', 'vertical', 'horizontal'], help: [
    ...
    dragHandle(e) {
        ...
        if ((this.getProp('mode') === 'vertical' && !e.traversing)) {
            // vertical
            this.setPercent(this.percent - 100 * (e.movementY / inertia * this.getProp('sensitivity')) / this.minDimension)

        } else if ((this.getProp('mode') === 'horizontal' && !e.traversing)) {
            // horizontal
            this.setPercent(this.percent + 100 * (e.movementX / inertia * this.getProp('sensitivity')) / this.minDimension)
