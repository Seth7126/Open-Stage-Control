...
class Vm {
    ...
    constructor() {
        ...
        this.globals.console = {
            ...console,
            log: (...msg)=>{
                msg = msg.map(m => typeof m == 'object' ? JSON.stringify(m, this.jsonReplacer) : m)
                this.getWidget().log(msg.join(' '))
            }
        }

        this.globals.setTimeout =
        this.globals.setInterval = ()=>{
    ...
    registerGlobals() {
        ...
        for (var k in this.globals) {
            this.sandbox.contentWindow[k] = this.globals[k]
        }
    ...
    jsonReplacer(key, value) {
        return value instanceof HTMLElement ? `&lt;${value.tagName}#${value.id}&gt;` : value
    }
