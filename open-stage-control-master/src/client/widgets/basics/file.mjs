...
export default class File extends Widget {
    ...
    static defaults() {
        ...
        return super.defaults().extend({
            style: {
                ...
                click: {type: 'boolean', value: false, help: 'Set to `true` to activate upon click event instead of tap'},
            },
            class_specific: {
    ...
    constructor(options) {
        ...
        this.on(this.getProp('click') ? 'click' : 'fast-click', (e)=>{

            if (e.capturedByEditor === true) return

            uiFilebrowser({
                extension: ext.replace(/^\.?(.*)$/, '$1'),
                directory: this.getProp('directory') === 'auto' ? this.lastDir : this.getProp('directory'),
