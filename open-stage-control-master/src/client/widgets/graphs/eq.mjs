...
class Eq extends StaticProperties(Plot, {logScaleX: false, logScaleY:false, smooth:true}) {
        ...
        return super.defaults().extend({
            ...
            class_specific: {
                filters: {type: 'array', value: '', help: [
                    ...
                    '- `poles`: if `type` is "highpass" or "lowpass", indicates the number of poles for the filter (if omitted or 0, a biquad filter with Q factor is used). Set to 1 for 6dB/octaves roll-off, 2 for 12dB/octaves, etc.',
    ...
    setupArrays() {
        ...
        this.freqs = new Float32Array(this.width)
        this.value = new Float32Array(this.width).fill(0)

        var i,
            min = this.getProp('rangeX').min,
            max = this.getProp('rangeX').max,
            width = this.width

        for (i = 0; i < width; ++i) {
            this.freqs[i] = mapToScale(i, [0, width], [min, max], -1, true)
        }
    ...
    resizeHandle(event){
        ...
        if (this.width !== this.value.length) {
            this.setupArrays()
            this.calcPending = true
            this.batchDraw()
        }
...
function biquadResponse(options, frequencyHz, filterResponse) {
    ...
    for (i = 0; i < len; i++) {
        ...
        w = mapToScale(frequencyHz[i], [0, Fs / 2], [0, Math.PI], -1, false),
