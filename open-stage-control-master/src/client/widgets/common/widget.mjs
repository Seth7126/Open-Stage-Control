...
class Widget extends EventEmitter {
    ...
    static defaults(Class=Widget) {
        ...
        var defaults = {
            ...
            style: {
                colorText: {type: 'string', value: 'auto', help: 'Text color. Set to "auto" to inherit from parent widget. Must be a valid css color.'},
                colorWidget: {type: 'string', value: 'auto', help: 'Widget\'s default accent color. Set to "auto" to inherit from parent widget. Must be a valid css color.'},
                colorStroke: {type: 'string', value: 'auto', help: 'Stroke color. Set to "auto" to use `colorWidget`. Must be a valid css color.'},
                colorFill: {type: 'string', value: 'auto', help: 'Fill color. Set to "auto" to use `colorWidget`. Must be a valid css color.'},
    ...
    constructor(options={}) {
        ...
        if (this.getProp('onValue')) {
            ...
            this.on('value-changed', (e)=>{
                if (e.widget === this && this.mounted && e.options.script !== false) {
    ...
    resolveProp(propName, propValue, storeLinks=true, originalWidget, originalPropName, context, recursive) {
        ...
        if (typeof propValue == 'string') {
            ...
            if (
                // json heuristic
                isJSON(propValue) &&
                // allow "false" string in label
                (propName !== 'label' || propValue === 'false') &&
                // don't coerce booleans and numbers as strings in objects
                (!recursive || (isNaN(propValue) && propValue != 'true' && propValue != 'false'))
            ) {
                try {
            ...
            for (let k in propValue) {
                propValue[k] = this.resolveProp(propName, propValue[k], storeLinks, originalWidget, originalPropName, context, true)
            }
        }
    ...
    updateProps(propNames, widget, options, updatedProps = []) {
        ...
        for (var propName of propNames) {
            ...
            if (
                !deepEqual(oldPropValue, propValue) ||
                (options && options.fromEditor && propName == 'value' && !deepEqual(oldPropValue, this.value))
            ) {
        ...
        if (options && options.fromEditor) {
            ...
            if (this.checkLinkedProps(propNames)) {
                ...
                widgetManager.trigger('prop-src-changed', {
                    id: this.getProp('id'),
                    props: unNotified,
                    widget: this,
                    options: options
