...
class Modal extends Panel {
    ...
    static defaults() {
        ...
        return super.defaults().extend({
            class_specific: {
                doubleTap: {type: 'boolean', value: false, help: 'Set to `true` to make the modal require a double tap to open instead of a single tap (allows scrolling when touching the modal button)'},
                click: {type: 'boolean', value: false, help: 'Set to `true` to make the modal open/close upon click event instead of tap (allows scrolling when touching the modal button)'},
            },
            geometry: {
    ...
    constructor(options) {
        ...
        this.on(this.getProp('click') ? 'click' : 'draginit',(e)=>{
            if ((e.target === this.popup || e.target === closer) && this.value == 1) {
                this.setValue(0, {sync:true, send:true})
            } else if (!this.getProp('doubleTap') && e.target === this.toggle) {
        ...
        if (this.getProp('doubleTap')) {
            doubleTap(this, ()=>{
                this.setValue(1, {sync:true, send:true})
            }, {element: this.toggle, click: this.getProp('click')})
        }
