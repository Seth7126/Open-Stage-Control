...
Gives focus to a widget (ie input widget). If `id` is omitted or falsy, currently focused element will be unfocused.
...
#### `scrollIntoView(id, option)`
- `id`: widget `id` as a string. Can be `"this"` to target the host widget, or `"parent"` to target the parent widget.
- `option`: `alignToTop` or `options` argument as defined in https://developer.mozilla.org/en-US/docs/Web/API/Element/scrollIntoView

Scroll containers to make widget(s) visible.
...
Opens the server file browser and pass selected file to a callback function (does not create nor read files by itself).
...
- `options`: object with the following keys:
    ...
    - `mode`: "save" or "open"
- `callback`: function called with 1 argument (file path)

#### `uploadFile(options, callback)`

Opens the client file upload prompt and pass the raw content of selected file to a callback function.

- `options`: object with the following keys:
    - `types`: equivalent of HTML attribute [`accept`](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Attributes/accept)
- `callback`: function called with 1 argument (file content)
